...
describe("library function", () => {
  it("should have read-only methods", done => {
    assert.throws(() => lq.range = null);
    done();
  });
  it("should have read-only props", done => {
    var oldProp = lq.empty;
    var err;
    try {
      lq.empty = null;
    }
    ...
    assert(lq.empty === oldProp && err);
    done();
  });
  it("should have enumerable methods", done => {
    assert(Object.keys(lq).indexOf("range") >= 0);
    done();
  });
});
...
describe("static methods", () => {
  it("can generate a range", done => {
    assert(lq.range(0, 3).sequenceEqual([0, 1, 2]));
    done();
  });
  it("can generate from generator functions", done => {
    var generator = lq.fromGenerator(function* () {
      for (var i = 0; i < 3; ++i) {
        yield i;
      }
    });
...
describe("instance methods", () => {
  it("can compare sequences", done => {
    assert(lq([1, 2, 3]).sequenceEqual([1, 2, 3]));
    assert(!lq([1, 2, 3]).sequenceEqual([1, 2, 4]));
    done();
  });
  it("can create arrays", done => {
    ...
    assert(lq(arr1).sequenceEqual(arr2));
    done();
  });
  it("can create maps", done => {
    var data = lq([[1, "chris"], [2, "aimee"], [5, "tilly"]]);
  ...
  it("can select sequences", done => {
    assert(lq([1, 2, 3]).select(i => i * 2).sequenceEqual([2, 4, 6]));
    done();
  });
  it("can flatten sequences of sequences", done => {
    var data = [[1, 2], [3, 4]];
    assert(lq(data).selectMany(d => d).sequenceEqual([1, 2, 3, 4]));
    done();
  });
  it("can aggregate", done => {
    assert(lq([1, 2, 3]).aggregate(0, (acc, curr) => acc + curr) === 6);
    done();
  });
  ...
  it("can concatenate sequences", done => {
    ...
    assert(data.sequenceEqual(lq.range(0, 10)));
    done();
  });
  it("can count sequences", done => {
    var data = lq.fromGenerator(function* () {
      for (var i = 0; i < 100; ++i) {
        yield i;
      }
    });
  ...
  it("can make sequences distinct", done => {
    assert(lq([3, 3, 2, 2, 1, 1]).distinct().sequenceEqual([3, 2, 1]));
    done();
  });
  it("can make sequences distinct using a selector", done => {
    var data = lq([[1, "chris"], [1, "dog"], [2, "aimee"], [5, "tilly"], [2, "cat"]]);
  ...
  it("can group a sequence", done => {
    ...
    assert(lq(groups[0]).sequenceEqual([1]));
    assert(lq(groups[1]).sequenceEqual([2, 3]));
    done();
  });
  it("can intersect a sequence with another", done => {
    var data = lq([1, 2, 3, 4, 5]);
